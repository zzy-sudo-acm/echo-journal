...
describe('Daily Snapshots', () => {
  ...
  it('should create a daily snapshot', async () => {
    ...
    // Verify snapshot data can be parsed
  ...
  it('should cleanup old snapshots keeping last 7', async () => {
    ...
    // Create 10 snapshots with different dates
    for (let i = 0; i < 10; i++) {
      const snapshot = {
        id: `snap-old-${i}`,
        ...
        size: 100,
        isPinned: false,
        data: JSON.stringify({ manifest: { entryCount: 1 }, entries: [], tags: [] }),
      }
  ...
  it('should never delete pinned snapshots', async () => {
    ...
    // Create 5 unpinned + 1 pinned snapshot
    for (let i = 0; i < 5; i++) {
